...
class BertMlmOnlyTfRecordsDynamicMaskProcessor(TfRecordsProcessor):
    ...
    - "use_multiple_workers" (bool): Specifies whether using multiple 
        workers with the Cerebras System for training
    - "mask_whole_word" (bool): Specifies whether to mask whole words
    - "max_sequence_length" (int): the maximum sequence length
    - "max_predictions_per_seq" (int): maximum number of masked LM predictions 
        per sequence
    - "masked_lm_prob" (float): Probability of generating a masked token
    - "mixed_precision" (bool): Specifies whether to generate float16 instead 
        of float32 tensors for mixed-precision training
